...
func OpenTunDevice(name, subnet string, dns string) (io.ReadWriteCloser, error) {
    return water.NewWintun(name, subnet, cfgGateway, dns, 16 * 1024 * 1024)
}

// func init() {
//     if systool.IsInitAdminPipe() {
//         systool.AdminPipe.FuncMap["wintun.CreateAdapter"] = func(args ...any) (any, error) {
//             guid, _ := windows.GUIDFromString("{1ba1e400-436f-4abf-b603-ef84c583d610}")
//             adapter, err := wintun.CreateAdapter(args[0].(string), "Wintun", &guid)
//             if err != nil {
//                 return nil, err
//             } else {
...
// func OpenTunDevice(name, subnet string, dns string) (io.ReadWriteCloser, error) {
//     if luid, err := systool.CallAsAdmin("wintun.CreateAdapter", name); err != nil {
//         return nil, err
//     } else {
//         log.Printf("luid: %f", luid)
//     }
